...
#define RANDOM 1
    ...
    //std::cout << "ID: " << initSituation.GetID() << std::endl;

    const std::vector<GameSituation>& path = Graph::Path(initSituation);
    std::cout << "Time spended: " << timer.ElapsedMillis() / 1000.0f << "s" << std::endl;
...
#if AUTO
         std::this_thread::sleep_for(std::chrono::milliseconds(AUTO));
         if (iter > 0) iter--;
         else {
             iter = path.size() - 1;
         }
